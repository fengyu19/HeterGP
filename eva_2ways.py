...
def acc_f1_over_batches(test_loader, PG1, PG2, gnn, answering, num_class, task_type, device):
    PG1 = PG1.to("cpu")
    PG2 = PG2.to("cpu")
    ...
    for batch_id, test_batch in enumerate(test_loader):
        test_batch_bfs, test_batch_dfs = test_batch
        prompted_graph_bfs = PG1(test_batch_bfs)
        graph_emb_bfs = gnn(prompted_graph_bfs.x, prompted_graph_bfs.edge_index, prompted_graph_bfs.batch)

        prompted_graph_dfs = PG2(test_batch_dfs)
