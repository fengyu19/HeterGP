...
from torch_geometric.datasets import TUDataset, Planetoid, WebKB, Actor, WikipediaNetwork, Amazon, Reddit, WikiCS
...
def load_data4graphLevelPretrain(dataname='CiteSeer', num_parts=200):
    dataset = TUDataset(root='./dataset/', name=dataname)
    data = dataset.data
    print(data)
...
def get_args():
    ...
    parser.add_argument('--induced_type', type=str)
...
def load_dataset(dataname):
    if dataname in ['CiteSeer', 'PubMed', 'Cora']:
    ...
    elif dataname in ['ENZYMES', 'MUTAG', 'PROTEINS', 'COX2', 'BZR']:
        dataset = TUDataset(root='./dataset/', name=dataname)
    elif dataname in ['texas', 'wisconsin', 'cornell']:
        dataset = WebKB(root='./dataset/', name=dataname)
    elif dataname in ['Actor']:
        dataset = Actor(root='./dataset/{}'.format(dataname))
    elif dataname in ['crocodile', 'chameleon', 'squirrel']:
        dataset = WikipediaNetwork(root='./dataset/', name=dataname)
        ...
        raise Exception('Unknown dataset: {}'.format(dataname))
    return dataset
